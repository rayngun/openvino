...
// Rewrites a FP16 Dequantize chain into an equivalent FP32 chain + a single f16 cast.
//
//   FQ(data, il, ih, ol, oh) -> Conv1(->int) -> Conv2(int->f16) -> [Sub(f16_zp)] -> Mul(f16_scale)
//                                   |
//                                   v
//   FQ(data, il, ih, ol, oh) -> Conv1(->int) -> Conv2(int->f32) -> [Sub(f32_zp)] ->
//       Mul(f32_scale) -> Conv(f32->f16)
